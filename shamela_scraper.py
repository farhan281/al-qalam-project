...
REPORTS_DIR  = os.path.join(OUTPUT_DIR, "reports")        # per-category reports live here
...
def _write_csv(path, rows):
    """Write rows to a UTF-8-sig CSV file with the shared field order."""
...
def generate_report(done):
    """
    Walk all .txt files in the output folder and write master and per-category CSVs.

    Writes:
      - shamela_output/report.csv
      - shamela_output/reports/report.csv
      - shamela_output/reports/<category>.csv
      - shamela_output/<category>/<category>.csv
    """
    ...
    _write_csv(os.path.join(REPORTS_DIR, "report.csv"), all_rows)
